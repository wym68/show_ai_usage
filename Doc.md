...
│
└── scripts/                           # ═══ 安装/构建脚本 ═══
    ├── install.sh                     # 安装脚本（支持源码/插件包双模式）
    ├── uninstall.sh                   # 卸载脚本
    └── build-plugin.sh                # 插件打包脚本（生成 .plasmoid）
```
...
- **插件包模式：** 检测到 `.plasmoid` 文件时，从包文件安装
- **源码模式：** 检测到 `package/` 目录时，从源码安装
...
```bash
./scripts/build-plugin.sh              # 打包到 dist/
./scripts/build-plugin.sh --output dir # 指定输出目录
```
