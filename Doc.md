...
**KDE Plasma 6 任务栏小部件**，监控 **订阅制 AI 服务**（OpenAI Codex、Claude Code、Kimi、MiniMax Token Plan）的滚动窗口使用额度和剩余配额。支持两种抓取方式，逐平台可选：**直连 API**（Claude / Kimi / MiniMax）与**浏览器自动化**（Codex 仅支持此方式）。
...
1. 按平台选择抓取方式：
   - **直连 API**（Claude / Kimi / MiniMax）：用本地 Token 直接调用接口，无需浏览器
   - **浏览器自动化**（Codex，及可选的其他平台）：通过 Playwright 启动隔离 Edge，用预先保存的登录态访问用量页面
2. 提取 5 小时 / 7 天滚动窗口使用百分比、剩余额度、重置时间
3. 写入本地 JSON 文件
4. KDE Plasmoid 定时读取并显示在任务栏上（悬停看精简用量、点击展开完整面板、点击平台名跳转官方用量页）
5. **所有凭据只在本地（`secrets.env` 权限 `0600`、浏览器登录态目录权限 `0700`），不上传任何第三方**
...
| **悬停精简** | 鼠标悬停面板即显示精简用量（百分比 + 重置时间），无需点击 |
| **弹出模式** | 点击展开完整面板：彩色进度条 + 百分比 + 剩余额度 + 重置时间 |
| **一键跳转用量页** | 弹出面板中点击平台名称，在系统浏览器打开该平台官方用量页 |
| **双抓取方式** | 直连 API（Claude/Kimi/MiniMax）或浏览器自动化（Codex），逐平台可选 |
| **凭据安全存储** | Token 写入 `secrets.env`（0600）、登录态目录 0700、`data.json` 0600 |
...
| **一键复制凭据命令** | Data Polling 标签页中每个提供商右侧可直接复制凭据命令（`--login` 或 `--set-token`） |
| **交互式安装引导** | `./scripts/install.sh` 逐平台引导选择抓取方式并录入凭据（可 `--no-onboard` 跳过） |
| **CLI 操作** | --login / --set-token / --oneshot / --daemon / --status / --debug 完整命令 |
...
│                                                              │
│  ┌─ Direct API（默认）─────────┐  ┌─ Browser ─────────────┐ │
│  │  Claude  → HTTP API (opt-in)│  │  Codex  → Playwright   │ │
│  │  Kimi    → HTTP API        │  │  (Claude/Kimi/MiniMax  │ │
│  │  MiniMax → HTTP API / mmx   │  │   可切换到此路径)       │ │
│  └────────────────────────────┘  └────────────────────────┘ │
│                                                              │
│  抓取方式逐平台可选；直抓失败时可回退浏览器（direct_fetch_browser_fallback）│
└────────────────────────┬──────────────────────────────────┘
                         │ 触发
┌────────────────────────┴───────────────────────────────────┐
...
1. systemd timer 每 5 分钟触发 → 执行 `--oneshot`（先经 `EnvironmentFile=secrets.env` 注入凭据）
2. Python Poller 按每平台的 `*_use_direct_fetch` 配置选择抓取路径：
   - **Direct API**（Claude / Kimi / MiniMax）：直接调用 HTTP 接口（MiniMax 无 Key 时可回退 `mmx` CLI），不启动浏览器
   - **Browser**（Codex，及切换为浏览器方式的其他平台）：启动隔离 Edge 浏览器访问仪表盘
3. 提取 5 小时 / 7 天滚动窗口使用百分比、额度、重置时间
...
│       │       ├── PollingConfig.qml   # 数据抓取配置 (启用/间隔/提供商/抓取方式/登录/Token)
│       │       ├── ProviderRow.qml     # 单个提供商配置行 (复选框+方式+登录/Token+命令)
│       │       ├── DisplayConfig.qml   # 显示配置 (模式/标签/颜色)
│       │       └── AdvancedConfig.qml  # 高级配置 (路径/主题)
│       │   # 注：四个配置页均包在 QQC2.ScrollView 内，窗口过小时按需出现横/竖滚动条
│       └── scripts/
│           └── sync_config.py         # Plasmoid ↔ poller 配置同步 + 凭据写入脚本
│
...
|------|------|
| `cli()` | 参数解析 + 命令分发 |
| `_handle_login(provider)` | 打开有头浏览器导航到指定平台首页等待登录。**TTY 下**按 Enter 保存；**无 TTY 时**（如从插件「一键登录」启动）改为等待用户关闭浏览器窗口后保存 |
| `_wait_for_browser_close(context)` | 无终端场景下轮询持久化上下文的页面，直到用户关闭浏览器（或超时），用于 GUI 启动的登录 |
| `_handle_set_token(provider)` | 用 `getpass` 隐藏输入读取直连凭据，写入 `secrets.env`（0600，不进命令历史）。仅 kimi/minimax/claude 有效 |
...
| `_poll()` | 按每平台配置分流：直连 provider 走 `fetch_direct()`，其余进浏览器队列（打开 Edge → 遍历 → 关闭）；直连失败可按 `direct_fetch_browser_fallback` 回退 |
...
| `load_config()` | 先调用 `load_secrets_env()` 注入凭据，再读取 `config.toml` 解析为 `Config` 对象 |
| `merge_cli_overrides()` | CLI 参数覆盖配置文件：生成新的 Config 实例 |
| `init_default_config()` | 写入带注释的默认配置到 `config.toml` |
| `load_secrets_env()` | 解析 `secrets.env`（`KEY=value`），以 `setdefault` 注入 `os.environ`（已存在的环境变量优先） |
| `write_secret(env_var, value)` | 写入/更新 `secrets.env` 中单个键，保留其余内容，文件权限设为 `0600` |

**凭据存储：** Token / API Key 统一存放在 `~/.config/show-ai-usage/secrets.env`（`KEY=value` env 格式，权限 `0600`），**绝不写入 `config.toml`**。`load_config()` 以「环境变量优先、文件兜底」的方式注入，使手动 `--oneshot` 与 systemd 服务（通过 `EnvironmentFile=`）解析到相同凭据。`SECRET_ENV_BY_PROVIDER` 映射各 provider → 环境变量名（codex 无直连凭据，故缺省）。
...
| `concurrent` | bool | false | 启用并行抓取（实验性） |
| `provider_timeout` | int | 60 | 单个 provider 抓取超时（秒，最小 10） |
| `direct_fetch_browser_fallback` | bool | false | 直抓失败时是否回退到浏览器抓取 |
| `claude_use_direct_fetch` | bool | false | Claude 抓取方式：true=直连 API，false=浏览器（默认浏览器） |
| `kimi_use_direct_fetch` | bool | true | Kimi 抓取方式：true=直连 API（默认），false=浏览器 |
| `minimax_use_direct_fetch` | bool | true | MiniMax 抓取方式：true=直连 API（默认），false=浏览器 |

**凭据字段**（建议改用 `secrets.env` / `--set-token`，环境变量优先）：

| 字段 | 环境变量 | 说明 |
|------|----------|------|
| `claude_code_access_token` | `CLAUDE_CODE_ACCESS_TOKEN` | 留空则直连自动读取 `~/.claude/.credentials.json` |
| `kimi_code_access_token` | `KIMI_CODE_ACCESS_TOKEN` | Kimi Code 访问令牌 |
| `minimax_api_key` | `MINIMAX_API_KEY` | MiniMax API Key；留空且装有 `mmx` CLI 时自动回退 |
| `minimax_api_base_url` | `MINIMAX_API_BASE_URL` | 默认 `https://api.minimax.io`，可选 `https://api.minimaxi.com` |

**`[network]`**
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `proxy` | string | "" | HTTP 代理地址，如 "http://127.0.0.1:7890" |
| `browser_channel` | string | "msedge" | 浏览器通道：msedge / chrome / chromium |

**`[notifications]`**
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `enabled` | bool | false | 启用桌面通知 |
| `warn_5h` | int | 80 | 5 小时用量通知阈值（0–100） |
| `warn_7d` | int | 80 | 7 天用量通知阈值（0–100） |

**`[locale]`**
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `timezone` | string | ""（自动检测） | 浏览器 IANA 时区 ID，如 "Europe/Brussels" |
| `language` | string | "zh-CN" | 浏览器语言标签 |

**`[paths]`**
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `data_dir` | string | `~/.local/share/show-ai-usage` | JSON 数据文件目录 |
| `browser_data_dir` | string | ""（= XDG 默认） | 隔离浏览器配置文件目录 |
...
- 独立的 `browser-data/` 用户数据目录（与系统浏览器完全隔离），保存登录 Cookie/会话，权限锁定为 `0700`
...
| 函数 | 作用 |
|------|------|
| `save_results(results, data_dir)` | 写入 JSON：`{ fetched_at, providers: [...] }`，文件权限设为 `0600`（仅属主可读，由同用户的 Plasmoid 读取） |
...
#### `poller/providers/direct.py` — 直连抓取协议

`DirectFetchProvider`（`typing.Protocol`，`runtime_checkable`）定义无需浏览器的直连接口。Claude / Kimi / MiniMax 同时实现 `BaseProvider`（浏览器路径）和该协议（直连路径）：

| 成员 | 说明 |
|------|------|
| `supports_direct_fetch: bool` | 是否具备直连能力（凭据可用等） |
| `fetch_direct(config) -> UsageData` | 直接调用 HTTP API / CLI 返回用量 |

`main.py._poll()` 按 `isinstance(provider, DirectFetchProvider)` + `supports_direct_fetch` + 每平台 `*_use_direct_fetch` 配置决定走直连还是浏览器队列。
...
- **直连 API（opt-in）：** `GET https://api.anthropic.com/api/oauth/usage`，携带 OAuth Bearer Token
  - **Token 解析顺序：** `claude_code_access_token`（env-first）→ 本机 `~/.claude/.credentials.json`（支持 `claudeAiOauth.accessToken` 等多种键名）
  - 由 `claude_use_direct_fetch` 控制，默认关闭（走浏览器）
- **浏览器抓取（默认）：** 导航到 `https://claude.ai/new#settings/usage`
  - **解析策略：** 中文模式（5小时/每周）→ 英文模式（"You've used X%" / "used X%"）→ 通用双百分比 fallback
  - 与 Codex 相同的 DOM 回退逻辑

#### `poller/providers/kimi.py` — Kimi 抓取器

- **直连 API（默认）：** `GET https://api.kimi.com/coding/v1/usages`，携带 `KIMI_CODE_ACCESS_TOKEN` Bearer Token
- **浏览器抓取：** 导航到 `https://www.kimi.com/code/console`，中文模式（5小时/周使用量）→ 通用百分比 fallback

#### `poller/providers/minimax.py` — MiniMax 抓取器

- **直连（默认）：** 优先用 `MINIMAX_API_KEY` 调用 `{base_url}` 接口；无 Key 时若 `mmx` CLI 在 PATH 上，回退执行 `mmx quota show --output json`（同一 JSON schema，免 Token）
- **浏览器抓取：** 导航到 `https://platform.minimaxi.com/console/usage`，中文模式（5小时/7天/本周）→ 英文模式（"5-hour" / "7-day" / "weekly" / "credits"）→ 通用 fallback
...
- **配置同步：** 当 Polling 配置（开关/间隔/提供商/每平台抓取方式）变更时，自动调用 `sync_config.py`（带 `--claude-method` / `--kimi-method` / `--minimax-method`）同步到 poller 的 `config.toml`
...
- **可跳转标题：** 卡片标题以 `Text.RichText` 渲染为超链接，点击经 `Qt.openUrlExternally()` 在系统浏览器打开该平台官方用量页（`providerUrl()` 维护 URL 映射，悬停显示手型光标）
...
| | `claudeFetchMethod` | String | Claude 抓取方式："browser"（默认）/ "direct" |
| | `kimiFetchMethod` | String | Kimi 抓取方式："direct"（默认）/ "browser" |
| | `minimaxFetchMethod` | String | MiniMax 抓取方式："direct"（默认）/ "browser" |
...
- 启用/禁用抓取开关、抓取间隔 SpinBox
- 由 4 个 `ProviderRow` 实例组成提供商列表，父页负责持久化与命令执行：
  - 收集各行勾选状态 → 重建 `enabledProviders` 字符串
  - 各行选定的方式写入隐藏字段 `claudeFetchMethod` / `kimiFetchMethod` / `minimaxFetchMethod`（`cfg_` 别名绑定到 kcfg），再由 `main.qml` 经 `sync_config.py --*-method` 翻译为 `*_use_direct_fetch`
- **就地执行**（`org.kde.plasma.plasma5support` 的 executable 引擎）：
  - 启动时读取 `~/.config/show-ai-usage/runtime.conf`（`PROJECT_DIR` / `UV`），解析项目与 uv 路径
  - `runLogin(id)`：`cd <PROJECT_DIR> && <uv> run python -m poller.main --login <id>`（无终端 → 关窗保存）；runtime.conf 缺失时提示改用终端
  - `saveToken(id, token)`：调用插件自带 `sync_config.py --save-secret <id> --token <token>`（系统 python3、纯标准库，无需 venv）写入 `secrets.env`
- 底部「状态」行回显执行结果；凭据**绝不**写入 kcfg/插件配置
- 使用 `property alias cfg_<name>` 自动绑定到 kcfg；整页包在 `QQC2.ScrollView` 内支持滚动

#### `package/contents/ui/config/ProviderRow.qml` — 单个提供商配置行

可复用组件，描述一个平台的配置行。输入属性由父页设置（`providerId` / `providerName` / `browserOnly` / `canToken` / `tokenOptional` / `noteBrowser` / `noteDirect`），内部以本地 `method` 属性派生命令与提示，**保证切换抓取方式时命令文本与说明实时刷新**：

| 区域 | 内容 |
|------|------|
| 第一行 | 启用复选框 + 抓取方式 ComboBox（`browserOnly` 时为固定标签「浏览器登录」） |
| 命令行 | 只读命令预览（`--login` / `--set-token`）+「复制」按钮 |
| 操作行 | 浏览器方式 →「一键登录」按钮；直连方式（`canToken`）→ Token 密码框 +「保存 Token」按钮 |
| 提示行 | 按当前方式显示对应说明文案 |

通过信号（`toggledEnabled` / `pickedMethod` / `requestLogin` / `requestSaveToken` / `requestCopy`）回调父页，自身不持久化、不执行命令。
...
#### `package/contents/scripts/sync_config.py` — 配置同步 + 凭据写入

纯标准库脚本（用系统 `python3` 运行，不依赖 poller 的 venv），由 Plasmoid 调用：

| 命令 | 作用 |
|------|------|
| `--enable --interval N --providers ... --{claude,kimi,minimax}-method {browser,direct}` | 写 `config.toml`（含 `*_use_direct_fetch`）并 `enable --now` systemd timer |
| `--disable` | 停止并禁用 timer |
| `--save-secret <provider> --token <token>` | 将凭据写入/更新 `secrets.env`（保留其余条目，权限 `0600`）；codex 无直连凭据时报错 |

> `--save-secret` 的逻辑与 `poller.config.write_secret()` 等价，但为标准库实现，便于配置页直接调用而无需 venv。`SECRET_ENV_BY_PROVIDER` 与 poller 端保持一致。
...
[Service]
Type=oneshot
# 凭据 env-first 加载；前导 "-" 使文件可选，缺失时单元仍能启动
...
- `@@PROJECT_DIR@@` / `@@UV_PATH@@` 是模板占位符，安装时被替换为真实路径
- `EnvironmentFile=`：加载 `secrets.env` 中的凭据（与手动 `--oneshot` 一致）
...
4. 写入 `~/.config/show-ai-usage/runtime.conf`（`PROJECT_DIR` + `UV` 路径，`0600`），供配置页定位并启动 poller（一键登录）
5. 替换 `@@PROJECT_DIR@@` / `@@UV_PATH@@` → 真实路径，复制 systemd 文件到 `~/.config/systemd/user/`
6. `systemctl --user daemon-reload` + `enable --now show-ai-usage.timer`
7. **交互式引导**（`run_onboarding`，TTY 下默认运行，可 `--no-onboard` 跳过）：展示抓取方式与隐私数据存储位置，逐平台选择抓取方式（浏览器 `--login` / 直连 `--set-token`），并将所选方式写入 `config.toml`
8. 显示安装摘要

可用选项：`--no-timer`（跳过 systemd timer）、`--no-onboard`（跳过引导）、`--dry-run`、`--prefix <path>`。
...
`install.sh` 默认启动**交互式引导**，逐平台让你选择抓取方式并录入凭据。也可手动配置——每个平台二选一（codex 仅支持浏览器）：

**方式一：浏览器登录。** 登录态保存在 `~/.local/share/show-ai-usage/browser-data/`（XDG 目录，权限 `0700`，与项目位置解耦）：
...
**方式二：直连 API（Claude / Kimi / MiniMax）。** 用 `--set-token` 安全录入凭据（输入隐藏、不进命令历史），写入 `secrets.env`（权限 `0600`）：

```bash
uv run python -m poller.main --set-token claude    # 留空可自动读取 ~/.claude/.credentials.json
uv run python -m poller.main --set-token kimi
uv run python -m poller.main --set-token minimax    # 装有 mmx CLI 时也可免 Token
```
...
| | 数据提供商 | 勾选要监控的平台，**即时生效** |
| | 抓取方式 | 每平台选择 浏览器 / 直连（codex 固定浏览器） |
| **Display** | 显示模式 | 0=5h+7d 都显示 / 1=仅 5h / 2=仅 7d |
| | 紧凑标签 | 面板上是否显示 provider 字母（O/C/K/M） |
| | 最大显示数 | 紧凑模式最多显示几个 provider |
| **Advanced** | 自定义数据路径 | 自定义 data.json 位置 |
| | 配色方案 | 默认 / 色盲友好 / 自定义 |
| | 自定义颜色 | 各阈值区间的颜色（#RRGGBB 格式） |

**Data Polling 标签页中的就地操作：**
每个提供商按所选抓取方式提供操作——浏览器方式有「一键登录」按钮（打开隔离浏览器，登录后关窗即保存）；直连方式有 Token 密码框 +「保存 Token」按钮（写入 `secrets.env`）。两种方式都同时显示对应命令（`--login` / `--set-token`）并提供「复制」按钮，底部「状态」行回显结果。一键登录依赖安装时写入的 `runtime.conf`。
...
**凭据文件（独立于 config.toml）：**
Token / API Key 不写入 `config.toml`，统一存放在 `~/.config/show-ai-usage/secrets.env`（`KEY=value` env 格式，权限 `0600`），由 `--set-token` 维护。`load_config()` 以「环境变量优先、文件兜底」注入，systemd 服务通过 `EnvironmentFile=` 加载同一文件。

```bash
# secrets.env 示例（由 --set-token 自动生成/更新）
CLAUDE_CODE_ACCESS_TOKEN=...   # 留空则直连读取 ~/.claude/.credentials.json
KIMI_CODE_ACCESS_TOKEN=...
MINIMAX_API_KEY=...            # 留空且装有 mmx CLI 时自动回退
```

**CLI 参数覆盖：**
```bash
# 临时修改（不影响配置文件）
uv run python -m poller.main --oneshot --interval 600 --providers codex claude
```
...
```bash
# 登录（打开有头浏览器手动登录，保存登录态）
uv run python -m poller.main --login [provider]

# 安全录入直连凭据（getpass 隐藏输入 → 写入 secrets.env 0600）
uv run python -m poller.main --set-token kimi       # kimi / minimax / claude
...
# 守护模式（持续轮询）
uv run python -m poller.main --daemon --interval 300
...
uv run python -m poller.main --show-config
uv run python -m poller.main --show-config --interval 600

# 帮助
uv run python -m poller.main --help
```
...
- **Data Polling：** 启用/禁用数据抓取、抓取间隔、选择监控的 AI 平台、逐平台抓取方式（浏览器/直连）、一键登录、Token 录入、命令复制
- **Display：** 显示模式、紧凑标签、最大显示数
- **Advanced：** 自定义数据路径、配色方案

### Plasmoid 预览效果

**悬停（精简用量）：** 鼠标悬停面板上的彩色圆角条，即弹出各平台的 5h / 7d 百分比与重置时间。

![悬停效果图](fig/悬停效果图.png)

**点击（完整面板）：** 点击展开完整弹窗——每个平台一张卡片（5h / 7d 进度条 + 重置时间），标题可点击跳转官方用量页，底部「刷新」按钮立即重新加载。
...
> **覆盖范围**：**Claude Code、Kimi、MiniMax** 均支持直接 API 抓取；**OpenAI Codex 仅支持 browser-backed（基于浏览器）抓取路径**。Kimi / MiniMax 默认直连，Claude 默认浏览器（可在配置中切换为直连）。

| 项 | 说明 |
|----|------|
| **抓取方式开关** | `claude_use_direct_fetch`（默认 false）/ `kimi_use_direct_fetch`（默认 true）/ `minimax_use_direct_fetch`（默认 true） |
| **Claude 凭据** | `CLAUDE_CODE_ACCESS_TOKEN`；留空则直连自动读取 `~/.claude/.credentials.json` |
| **Kimi 凭据** | `KIMI_CODE_ACCESS_TOKEN`（建议经 `--set-token kimi` → secrets.env） |
| **MiniMax 凭据** | `MINIMAX_API_KEY`；留空且 `mmx` CLI 在 PATH 上时回退 `mmx quota show --output json`，免 Token |
...
| **MiniMax 积分余额** | 直抓返回的"mmx quota"/订阅积分与浏览器端一致，但目前不区分订阅积分、充值积分、赠送积分 |

未配置对应凭据时，直连会报告缺少凭据；只有开启 `direct_fetch_browser_fallback = true` 时才会回退到浏览器抓取。`--show-config` 输出会自动脱敏这些凭据字段，避免泄露。
...
uv run python -m poller.main --debug --providers claude
```
...
7. 在 `package/contents/ui/config/PollingConfig.qml` 中新增一个 `ProviderRow` 实例（并在 `rebuildProviders()` / `applyProvidersString()` 里纳入该行）；若支持直连，还需在 `config.py`、`main.xml`、`main.qml` 的方法同步链中加入对应 `*FetchMethod` / `*_use_direct_fetch`
...
> 直连方式失败时，先确认凭据：运行 `--set-token <provider>` 录入，或检查 `secrets.env` / 环境变量。Claude 直连无数据时确认本机存在 `~/.claude/.credentials.json`，MiniMax 可改用 `mmx` CLI 或填入 API Key。临时排查时也可设 `direct_fetch_browser_fallback = true` 回退浏览器。
...
```bash
uv run python -m poller.main --oneshot --providers codex
```
...
### Q: 凭据保存在哪里？卸载后需要重新登录吗？

A：取决于抓取方式：
- **浏览器登录态**：`~/.local/share/show-ai-usage/browser-data/`（权限 `0700`）
- **直连 Token / API Key**：`~/.config/show-ai-usage/secrets.env`（权限 `0600`，由 `--set-token` 维护）

卸载行为：
- 标准卸载（`./uninstall.sh`）：**保留**登录态与凭据，重新安装后无需重新配置
- 完全清理（`./uninstall.sh --purge` / `--purge-all`）：**删除**配置、数据与凭据，需要重新 `--login` / `--set-token`
