...
│                                                              │
│  ┌─ Direct API ───────────────┐  ┌─ Browser ─────────────┐ │
│  │  Kimi    → HTTP API        │  │  Codex  → Playwright   │ │
│  │  MiniMax → HTTP API        │  │  Claude → Playwright   │ │
│  └────────────────────────────┘  └────────────────────────┘ │
│                                                              │
│  (直抓失败时，Kimi/MiniMax 可回退到浏览器路径)                  │
└────────────────────────┬──────────────────────────────────┘
                         │ 触发
┌────────────────────────┴───────────────────────────────────┐
...
2. Python Poller 按 provider 类型选择抓取路径：
   - **Direct API**（Kimi / MiniMax）：直接调用 HTTP 接口，不启动浏览器
   - **Browser**（Codex / Claude）：启动隔离 Edge 浏览器访问仪表盘
...
| `kimi_code_access_token` | string | "" | Kimi Code 访问令牌（环境变量 `KIMI_CODE_ACCESS_TOKEN`） |
| `minimax_api_key` | string | "" | MiniMax API Key（环境变量 `MINIMAX_API_KEY`） |
| `minimax_api_base_url` | string | "https://api.minimax.io" | MiniMax 接口基础地址 |
| `claude_code_access_token` | string | "" | Claude Code 访问令牌（环境变量 `CLAUDE_CODE_ACCESS_TOKEN`，当前未启用直抓） |
| `claude_use_direct_fetch` | bool | false | 启用 Claude 直抓（默认关闭，始终使用浏览器抓取） |
...
> **阶段限制**：目前 **Kimi 与 MiniMax** 支持直接 API 抓取；**OpenAI Codex 与 Claude Code 保持 browser-backed（基于浏览器）抓取路径**。
...
未配置对应凭据时，Kimi / MiniMax 直抓会报告缺少凭据；只有开启 `direct_fetch_browser_fallback = true` 时才会回退到浏览器抓取。`--show-config` 输出会自动脱敏这些凭据字段，避免泄露。
